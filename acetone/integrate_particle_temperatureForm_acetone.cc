...
int main(void)
{
    ...
    //Numerics Section
    float dt = 1e-6; //timestep size(seconds)
    ...
    double lambda_g = 0.025652; //carrier gas thermal conductivity
    double cp_g = 1006.1; //heat capacity of gas J/kgK
    ...
    double R_v = 150.0; //gas constant for water vapor
    ...
    double mdot=0.0;
    double m_gas = r_g*fvolpp;
    double m_vapor = Y_g*m_gas/(1-Y_g); //initial mass of water
    ...
    for(int i = 0;i<IMAX;i++)
    {
        ...
        double dm = -mdot*dt;
        cout <<"Mdot: "<<mdot<<"\tAdded Vapor mass: "<<dm<<endl;
        m_vapor= m_vapor + dm;
...
double integrateMassEnergy(
    ...
    const double Psat = 10.0e3; //hardcoded saturatoin pressure @ 272K (~0C)
    ...
    if(T_p < 178) // Chris Hardcoded triple point for acetone Check if colder than triple point
    ...
    double hvap = 564e3; //Chris heat of vaporization of acetone at 272K (J/kgK)
    ...
    for(iter=0;iter<ITMAX;++iter) {
        ...
        cout<<"T: "<<T1<<"\tf2: "<<f2<<"\tNu: "<<Nu<<"\tCp_g: "<<cp_g<<"\tPr: "<<Pr_g<<"\ttau_p: "<<tau_p<<"\tC_liquid: "<<p.get_c()<<"\tTerm1Coeff: "<<fudge*f2*Nu*cp_g/(3.*Pr_g*tau_p*p.get_c())<<"\tTerm2Coeff: "<< hvap*mdot/(max(p.mass[0],1e-30f)*p.get_c())<<"\tBeta: "<<beta_b<<"\tMass"<<p.mass[0]<<"\tMdot"<<p.mdot<<endl;
    ...
    const double nPsat = 10e3; //Chris hardcoded saturation pressure @ 272K 
