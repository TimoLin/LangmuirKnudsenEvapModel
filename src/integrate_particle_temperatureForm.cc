...
struct saturation_data
{
    ...
    int count_data_file_lines()
    {
        int number_of_lines = 0;
        std::string line;
        std::ifstream myfile(data_file_name.c_str());
    ...
    int count_data_file_columns()
    {
        int number_of_columns = 0;
        std::string line;
        cout << "Filename to open: "<<data_file_name<<endl;
        std::ifstream myfile(data_file_name.c_str());
    ...
    void read_table(string file_name)
    {
        ...
        int num_lines = count_data_file_lines();
        int num_columns = count_data_file_columns();

        cout << "num Lines" <<num_lines<<" Num Columns: "<<num_columns<<endl;
...
class user_data
{
    ...
    public:
        //Numerics Section
        float dt; //timestep size(seconds)
        int IMAX; //Number of timesteps to take
        ...
        float T_g; //Temperature of gas (Kelvin)
        double P_g; //Pressure of gas (Pascals)
        double Y_g; //Mass Fraction of vapor in carrier gas
        double mu_g; //Viscosity of gas (Pascals*seconds)
        double Sc_g; //Schmidt number for gas diffusing into air http://webserver.dmt.upm.es/~isidoro/dat1/Mass%20diffusivity%20data.pdf
        double R_g; //Specific gas constant for carrir(air), J/kgK
        float u_g; //background fluid velocity(m/s) 
        double lambda_g; //carrier gas thermal conductivity
        double cp_g; //heat capacity of gas J/kgK

        double fvolpp; //Volume of container that droplet is in m^3

        //Define droplet quantities
        double D_p; //initial droplet diameter(meters)
        double T_p; //Kelvin
        double r_p; //kg/m^3 
        double cp_p; //heat capacity of the liquid particle J/kgK

        //Defined droplet vapor properties
        double R_v; //gas constant for vapor phase
...
int main(void)
{
    ...
    int IMAX = input_data.IMAX; //Number of timesteps to take

    //Define background gas quantities(Properties evaludated at the wet-bulb temperature of 420K
    //Twb = 137*(T_B/373.15K)^(0.68)*log10(T_G) - 45K
    float T_g = input_data.T_g; //Temperature of gas (Kelvin)
    double P_g = input_data.P_g; //Pressure of gas (Pascals)
    double Y_g = input_data.Y_g; //Mass Fraction of vapor in carrier gas
    double mu_g = input_data.mu_g; //Viscosity of gas (Pascals*seconds)
    double Sc_g = input_data.Sc_g; //Schmidt number for gas diffusing into air http://webserver.dmt.upm.es/~isidoro/dat1/Mass%20diffusivity%20data.pdf
    double R_g = input_data.R_g; //Specific gas constant for carrir(air), J/kgK
    float u_g = input_data.u_g; //background fluid velocity(m/s) 
    double lambda_g = input_data.lambda_g; //carrier gas thermal conductivity
    double cp_g = input_data.cp_g; //heat capacity of gas J/kgK

    double fvolpp = input_data.fvolpp; //Volume of container that droplet is in m^3

    //Define droplet quantities
    double D_p = input_data.D_p; //initial droplet diameter(meters)
    double T_p = input_data.T_p; //Kelvin
    double r_p = input_data.r_p; //kg/m^3 
    double cp_p = input_data.cp_p; //heat capacity of the liquid particle J/kgK

    //Defined droplet vapor properties
    double R_v = input_data.R_v; //gas constant for vapor phase
